...
```{r}
#Grunnen til at man kan se alle verdiene når man bruker binwidth=0.1 og ikke med binwidth=2 er på grunn av at den ikke har plass til like mange intervaller. Jo mindre binwidth er jo flere intervaller vil vi se fordi dem har plass til flere og trenger da ikke å sla de sammen.
```
...
df %>%
  ...
  ggplot(aes(value)) +
  geom_histogram() +
  facet_wrap(~ sysselsettingsgruppe) +
  theme_minimal()
```
