...
Den siste ligningen viser konsumprisindeksen i prosent for hver måned. For å finne hver måned blir den delt på Pt-12. For at svare skal vise prosent andel ganger vi resultatet med 100.
...
as.Date(df$dato, format, tryFormats = c('%Y-%m-%d'))
...
Prisindeksen bygges opp på samme måte. Vi har verdiene til prisindeksen samt endring og inflasjon. Dette gir oss muligheten til å beregne vekst over tid eller mellom geografiske posisjoner. Siden vi får noen avvik er ikke totalindeksen helt "riktig", men den kan være rådgivende brukelig.
...
df %>%
  ...
  ggplot(aes(x = dato, y = value, color = konsumgruppe)) +
  geom_point() +
  xlab('Tid') +
  ylab('Verdi') +
  theme_minimal()
...
Konsumgruppen "Elektrisitet inkludert nettleie" har steget kraftig de siste årene. Dette kan vi spesielt legge merke til i 2022. En av de største grunnene til dette er nok på grunn av strømkrisene som har forregått de siste årene. Økningen startet i år 2021, hvor vi også hadde et regjerrings skiftet.
...
På grunn av høyere kratpriser har det inndirekte bidrat til konsumprisvekst. Prisene på varer og tjenester har steget kraftig de siste årene.Høyere kraftipriser bidrar indirekte til høyere konsumpriser. Som i følge av økningen på strøm må produsentene betale mer til å produsere samme mengde varer og får å kompensere for det må vi som konsumere betale mer selv om vi ikke har høyere forbrukt. Dette gjør at konsumprisveksten stiger.
