...
Grunnen til at de utelukker å bruke verdiene til boligtjenester, offentlig etterspørsel og inntekter fra olje og gass har flere grunner. Etterspørselen og inntekten fra olje og gass er variene verdi som gjør at det blir vanskeligere å få sammenlignenet verdiene globalt. Olje og gass blir også beregnet som noe som blir eid av hele landet, og ikke enkelt personer. Dette gjør at verdien har høye grunn rente. Boligtjenester blir derimot ikke tatt med ettersom mye av byggingen innebærer offentlige sektorer som gjør at det blir vanskeligere å regne ut betalingsviljen.
...
```{r}
Markedsrettet <- Markedsrettet %>%
  mutate(Value = value - Boligpris$value) 
 
Markedsrettet <- Markedsrettet %>%
  mutate(Value_1 = Value/ Tabell_09174$value)

Markedsrettet <- Markedsrettet %>%
  mutate(Value_prosent = Value_1/100)
```

```{r}
Markedsrettet %>%
  ggplot(aes(Value_prosent)) +
  geom_histogram(binwidth=0.05) +
  theme_minimal()
```

For å regne gjennomsnitt av verdien innenfor en perioden legger du sammen alle verdiene for å deretter dele produktet på hvor mange år som blir brukt i perioden. Derfor blir det å se slikt ut:
